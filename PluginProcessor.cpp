...
juce::AudioProcessorValueTreeState::ParameterLayout MattJamesCW3AudioProcessor:: createParameters()
{
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>("FDBK_GAIN", "Feedback Gain",0.0f,1.0f,0.5f));
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>("MIX", "Dry/Wet Mix",0.0f,1.0f, 0.5f));
    
    parameters.push_back(std::make_unique<juce::AudioParameterInt>("DELAY_TOTAL", "Total delay time (N+M samps)",0,176400,44100));
    parameters.push_back(std::make_unique<juce::AudioParameterInt>("DELAY_TAP", " delay tap time (N samps)",0,88200,1));
